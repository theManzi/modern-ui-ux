...
export default function Hero() {
  ...
  const currentFloatingCord = floatingCard[activeTab];
    ...
    <section className="relative min-h-screen flex items-center justify-center pt-16 sm:pt-20 px-4 sm:px-6 lg:px-8 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto text-center relative w-full">
        <div className="max-w-7xl mx-auto flex flex-col lg:grid lg:grid-cols-2 gap-6 sm:gap-8 lg:gap-12 items-center relative">
          <div>
            <div className="inline-flex items-center space-x-2 p-3 sm:p-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-full mb-4 sm:mb-6 animate-in slide-in-from-bottom duration-700">
              <Sparkles className="w-4 h-4 text-blue-400" />
              <span className="text-xs sm:text-sm text-blue-300">
                Introducing CodeFlow IA
              </span>
            </div>
          ...
          <div className="relative order-2 w-full">
            <div className="relative bg-white/5 backdrop-blur-xl rounded-xl sm:rounded-2xl p-4 sm:p-4 shadow-2xl border border-white/10">
              ...
              <div
                className={`hidden lg:block absolute bottom-4 right-4 transform translate-x-8 translate-y-8 w-72 ${currentFloatingCord.bgColor} backdrop-blur-2xl rounded-lg p-4 border border-white/20 shadow-2xl`}
              >
                <div className="flex items-center space-x-2 mb-2">
                  <div
                    className={`w-6 h-5 ${currentFloatingCord.iconColor} flex items-center justify-center text-sm font-bold`}
                  >
                    {currentFloatingCord.icon}
                  </div>
                  <span
                    className={`text-sm font-medium ${currentFloatingCord.textColor}`}
                  >
                    {currentFloatingCord.title}
                  </span>
                </div>

                <div
                  className={`text-sm text-left ${currentFloatingCord.contentColor}`}
                >
                  {currentFloatingCord.content}
                </div>
              </div>
            </div>
          </div>
